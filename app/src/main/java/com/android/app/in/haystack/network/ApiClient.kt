...
object ApiClient {
    ...
    val retrofitService: ApiInterface by lazy { getClient().create(ApiInterface::class.java) }
    ...
    fun <S> createBearerAuthService(service: Class<S>, canCache: Boolean): S {
        ...
        val bearerAuthInterceptor = BearerAuthenticationInterceptor(token, canCache)
        val retrofit = getRetrofit(bearerAuthInterceptor)
